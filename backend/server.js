...
function deal(r){const d=shuffle(deck());for(let i=0;i<r.round;i++)for(const p of r.players)p.hand.push(d.pop());r.players.forEach(p=>p.hand.sort((a,b)=>SUITS.indexOf(a.suit)-SUITS.indexOf(b.suit)||a.value-b.value));r.phase="BIDDING";r.currentPlayerId=r.leaderPlayerId}
function reset(r){r.players.forEach(p=>{p.hand=[];p.bid=null;p.tricksWon=0});r.table=[];r.lastTrick=[];r.lastTrickWinnerId=null;r.roundScores=null;r.currentPlayerId=null;r.phase="DEALING"}
function roundEnd(r){r.roundScores=r.players.map(p=>{const score=p.bid===p.tricksWon?10+p.tricksWon:-10*Math.abs(p.bid-p.tricksWon);p.totalScore+=score;return {playerId:p.id,name:p.name,bid:p.bid,tricksWon:p.tricksWon,score,totalScore:p.totalScore}});r.history.push({round:r.round,scores:r.roundScores});if(r.round===7){r.phase="GAME_RESULT";r.final=r.players.map(p=>({playerId:p.id,name:p.name,totalScore:p.totalScore}))}else r.phase="ROUND_RESULT";r.currentPlayerId=null}
...
io.on("connection",s=>{
 s.on("createRoom",({name},cb)=>{const r={code:code(),hostId:s.id,phase:"WAITING",round:1,players:[{id:s.id,name:String(name||"Gracz").trim().slice(0,20)||"Gracz",seat:1,hand:[],bid:null,tricksWon:0,totalScore:0}],leaderPlayerId:s.id,currentPlayerId:null,table:[],lastTrick:[],lastTrickWinnerId:null,roundScores:null,history:[],final:null};rooms.set(r.code,r);s.join(r.code);cb?.({ok:true,code:r.code});emit(r)});
 ...
 s.on("startGame",cb=>{const r=roomOf(s.id);if(!r)return cb?.({ok:false,error:"Nie jesteś w pokoju."});if(r.hostId!==s.id)return cb?.({ok:false,error:"Tylko host może rozpocząć."});if(r.players.length<2)return cb?.({ok:false,error:"Potrzeba co najmniej 2 graczy."});r.round=1;r.leaderPlayerId=r.hostId;reset(r);deal(r);cb?.({ok:true});emit(r)});
 s.on("submitBid",({bid},cb)=>{const r=roomOf(s.id),p=player(r,s.id);if(!r||!p)return cb?.({ok:false,error:"Brak pokoju."});if(r.phase!=="BIDDING")return cb?.({ok:false,error:"Nie trwa licytacja."});if(p.bid!==null)return cb?.({ok:false,error:"Deklaracja została już zatwierdzona."});const n=Number(bid);if(!Number.isInteger(n)||n<0||n>r.round)return cb?.({ok:false,error:`Wybierz liczbę od 0 do ${r.round}.`});p.bid=n;const next=r.players[p.seat%r.players.length];if(r.players.every(x=>x.bid!==null)){r.phase="PLAYING";r.currentPlayerId=r.leaderPlayerId}else r.currentPlayerId=next.id;cb?.({ok:true});emit(r)});
 ...
 s.on("nextRound",cb=>{const r=roomOf(s.id);if(!r||r.hostId!==s.id||r.phase!=="ROUND_RESULT")return cb?.({ok:false,error:"Nie można rozpocząć następnej rundy."});r.round++;r.leaderPlayerId=r.lastTrickWinnerId||r.leaderPlayerId;reset(r);deal(r);cb?.({ok:true});emit(r)});
